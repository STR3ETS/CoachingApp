@extends('layouts.app')
@section('title','Plan bewerken')

@section('content')
<h1 class="text-2xl font-semibold mb-6">
  Plan bewerken voor {{ $client->user->name ?? 'Client #'.$client->id }}
</h1>

<form method="POST"
      action="{{ route('coach.plans.update', $plan) }}"
      x-data="planGridBuilder({{ (int)$plan->weeks }}, @js($plan->plan_json))"
      x-init="init()"
      class="max-w-5xl">
    @csrf
    @method('PUT')
    ...
    <section class="p-5 bg-white rounded-3xl border">
      ...
      <div class="mt-4">
        <div class="flex items-center justify-between mb-2">
          <h3 class="font-medium">Sessies</h3>
          <button type="button" class="text-sm underline" @click="addRow(w)">+ rij</button>
        </div>
                ...
                <tr class="border-t">
                  ...
                  <td class="p-2">
                    <button type="button" class="px-2 py-1 border rounded"
                            @click="removeFlatRow(w, idx)">X</button>
                  </td>
                </tr>
              </template>
            </tbody>
...
@push('scripts')
<script>
function planGridBuilder(defaultWeeks, initialPlan = {}) {
  return {
    title: @js($plan->title),
    ...
    // sessie-notities per week/dag
    sessionNotes: {}, // { [week]: { [day]: 'text' } }
    ...
    init(){
      // Prefill met bestaande plan_json
      this.plan = (initialPlan && typeof initialPlan === 'object') ? initialPlan : {};

      // Bepaal aantal weken op basis van bestaande data (fallback naar default)
      const weekNums = Object.keys(this.plan)
        .map(k => Number(String(k).replace('week_','')))
        .filter(n => !isNaN(n));
      const existingWeeks = weekNums.length ? Math.max(...weekNums) : 0;
      if (existingWeeks > 0) this.weeks = existingWeeks;

      // Zorg dat elke week bestaat en sessies array heeft
      for (let i=1; i<=this.weeks; i++) {
        this.ensureWeek(i);

        // Zet bestaande sessie-notities in sessionNotes
        const wk = this.plan['week_'+i];
        (wk.sessions || []).forEach(s => {
          if (s.day) this.setSessionNote(i, s.day, s.note || '');
        });
      }
    },
    ...
    // Flatten rijen voor de grid (per oefening)
    flatRows(w){
      this.ensureWeek(w);
      const key = 'week_'+w;
      const rows = [];
      ...
      for (let i=1; i<=this.weeks; i++){
        ...
        if (Array.isArray(v.sessions)) {
          v.sessions = v.sessions.map(s => ({
            day: s.day,
            note: this.getSessionNote(i, s.day),
            exercises: (s.exercises || []).map(e => ({
              name: e.name || '',
